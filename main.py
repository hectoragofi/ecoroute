...
def show_data():
    if request.method == 'POST':
        ...
        if departureCoordinates is None or arrivalCoordinates is None:
            empty = True
            valid_departure = False
            valid_arrival = False
            # Handle other cases or errors
            messagepop = {'message': 'Port names are wrong. Please check them'}
            return render_template('index.html', show_recommended_info=False, valid_departure=valid_departure,
                                   valid_arrival=valid_arrival, mess=messagepop, show_popup=True)
        elif departure_port.strip() and arrival_port.strip():
            empty = False
            valid_departure = True
            valid_arrival = True
        else:
